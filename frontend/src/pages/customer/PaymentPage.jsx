...
export default function PaymentPage() {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      await ordersApi.requestPayment(orderId, selectedMethod === 'momo' ? 'momo' : 'bank_transfer').catch(() => undefined)
    ...
    <div className="space-y-4">
      {activeUnpaidOrder && (
        <div className="card p-4">
          ...
          <p className="text-white font-medium">Tổng: {(activeUnpaidOrder.totalAmount || 0).toLocaleString('vi-VN')}₫</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium border ${
              selectedMethod === 'momo' ? 'border-brand-500 bg-brand-500/10 text-brand-400' : 'border-gray-700 bg-surface-elevated text-gray-300'
            }`}
          >
            Momo
          </button>
          <button
            ...
            className={`px-3 py-2 rounded-lg text-sm font-medium border ${
              selectedMethod === 'bank' ? 'border-brand-500 bg-brand-500/10 text-brand-400' : 'border-gray-700 bg-surface-elevated text-gray-300'
            }`}
          >
      ...
      {selectedMethod === 'momo' ? (
        <div className="card p-4 space-y-2">
          ...
          {instructions?.momo?.qrImageUrl ? (
            <img src={instructions.momo.qrImageUrl} alt="Momo QR" className="w-56 max-w-full rounded-lg border border-gray-700" />
          ) : (
            <p className="text-sm text-yellow-400">Chưa cấu hình ảnh QR Momo.</p>
          )}
          {instructions?.momo?.accountName && <p className="text-sm text-gray-400">Tên ví: {instructions.momo.accountName}</p>}
          {instructions?.momo?.accountNumber && <p className="text-sm text-gray-400">SĐT ví: {instructions.momo.accountNumber}</p>}
        </div>
      ) : (
        <div className="card p-4 space-y-1 text-sm">
          ...
          <p className="text-gray-300">
            Số tài khoản: <span className="text-white">{instructions?.bank?.accountNumber || 'Chưa cấu hình'}</span>
          </p>
        </div>
      )}
          ...
          <div className="w-full max-w-md rounded-xl bg-surface-card border border-gray-700 p-4 space-y-3">
            ...
            {ratingDone ? (
              <div className="space-y-3">
                ...
                <button type="button" onClick={() => setRatingModalOrderId(null)} className="btn-primary w-full py-2">
                  Đóng
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setRatingModalOrderId(null)} className="btn-secondary flex-1 py-2">
                    Để sau
                  </button>
                  <button
                    type="button"
