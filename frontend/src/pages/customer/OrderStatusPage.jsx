  ...
  const history = useMemo(() => {
    ...
    visibleOrders.forEach((order) => {
      order.items.forEach((oi, idx) => {
        ...
        const id = typeof mi === 'object' && mi && '_id' in mi ? mi._id : `${order._id}-${idx}`
        const name = typeof mi === 'object' && mi && 'name' in mi ? mi.name : `Món #${idx + 1}`
        const current = map.get(id)
        if (current) current.quantity += oi.quantity
        else map.set(id, { name, quantity: oi.quantity })
      })
    })
    ...
    <div className="space-y-6">
      {visibleOrders.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400">
          ...
          <button type="button" onClick={() => navigate(`/table/${tableId}`)} className="btn-primary mt-4">
            Xem menu
          </button>
        </div>
      ) : (
        <>
                ...
                {order.items.map((oi, idx) => {
                  ...
                  return (
                    <li key={idx} className="text-sm">
                      ...
                      {oi.note && <p className="mt-1 text-xs text-gray-500 italic">Ghi chú: {oi.note}</p>}
                    </li>
                  )
                })}
              </ul>
          ...
          {history.length > 0 && (
            <div className="card p-4">
              <p className="text-sm font-medium text-white mb-3">Lịch sử món đã gọi</p>
              <ul className="space-y-1 text-sm">
                {history.map((h) => (
                  <li key={h.id} className="flex justify-between text-gray-300">
                    <span>{h.name}</span>
                    <span>x{h.quantity}</span>
                  </li>
                ))}
              </ul>
            </div>
