...
export default function KitchenOrders() {
  ...
  const refresh = () => {
    ordersApi.listAll().then((list) => setOrders(list.filter((o) => o.status !== 'served' && o.status !== 'cancelled')))
  }
        ...
        return (
          <div key={order._id} className="card p-4">
            <div className="flex justify-between items-start mb-3">
              <span className="font-medium text-white">Bàn {tableName}</span>
              <span
                className={`px-2 py-1 rounded-lg text-xs font-medium ${
                  order.status === 'ready' ? 'bg-green-500/20 text-green-400' : 'bg-brand-500/20 text-brand-400'
                }`}
              >
