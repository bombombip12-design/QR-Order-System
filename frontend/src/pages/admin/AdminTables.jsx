...
export default function AdminTables() {
  ...
  const load = () => {
    // Luong khoi tao/truy hoi: lay danh sach ban, sau do lay don theo tung ban de hien thi "don dang hoat dong".
    adminApi.tables
      .list()
      .then((tables) => {
        setList(tables || [])
  ...
  useEffect(() => {
    // Chay 1 lan khi vao man hinh Admin > Ban.
    load()
  }, [])

  const openCreate = () => {
    // Bat dau che do tao moi: reset toan bo form.
    setMessage(null)
    setCreating(true)
    setEditing(null)
    setName('')
  ...
  const openEdit = (t) => {
    // Bat dau che do chinh sua: do du lieu ban hien tai len form.
    setMessage(null)
    setEditing(t)
    setCreating(false)
    setName(t.name)
  ...
  const save = async () => {
    // Nhan "Luu" trong popup xac nhan -> vao day.
    // Neu dang edit thi PATCH /api/admin/tables/:id, neu dang create thi POST /api/admin/tables.
    if (!name.trim()) {
    ...
    try {
      ...
      setCapacity('')
      setOrderUrl('')
      // Sau khi tao/sua thanh cong, tai lai danh sach + orders de UI dong bo voi DB.
      load()
    } catch (e) {
  ...
  const remove = async () => {
    // Xoa ban: DELETE /api/admin/tables/:id, xong se reload danh sach.
    ...
    <div className="space-y-6">
          ...
          <div className="w-full max-w-sm rounded-xl bg-surface-card border border-gray-700 p-4 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  setConfirmSave(false)
                  // Popup chi xac nhan; nghiep vu luu that su nam trong ham save().
                  await save()
                }}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((t) => {
          ...
          // activeOrders dung de thong bao nhanh tinh trang cua ban trong card.
