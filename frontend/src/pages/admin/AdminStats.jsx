...
/** Mau cho bieu do (top 5 mon ban chay) */
const CHART_COLORS = [
...
export default function AdminStats() {
  ...
  useEffect(() => {
    // Luong thong ke: GET /api/admin/stats -> { revenue, totalOrders, topItems }
    // Backend tinh tu toan bo Order trong DB; frontend chi hien thi + ve bieu do.
    adminApi.stats
      .get()
      .then(setStats)
      .catch(() => setStats(null))
  ...
  // Du lieu bieu do cot: ten mon vs tong so luong da ban (theo tat ca don).
  const barData = {
  ...
  // Bieu do tron: cung du lieu topItems, tooltip hien them % tren tong luong top 5.
  const doughnutData = {
    labels: stats.topItems.map((i) => i.name),
    datasets: [
      {
  ...
  const chartOptions = {
    // Tooltip ghi "X don" (thuc chat la tong so luong mon da ban trong cac don).
    responsive: true,
