      ...
      {(creating || editing) && (
        <div className="card p-4 space-y-3">
          ...
          <input
            ...
            disabled={!!editing}
          />
          {editing && <p className="text-xs text-gray-500">Không thể đổi email khi sửa. Xóa và tạo mới nếu cần.</p>}
          <input
            type="tel"
        ...
        {list.map((u) => (
          <li key={u._id} className="card p-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="px-2 py-1 rounded-lg bg-surface-elevated text-gray-300 text-sm">{roleLabel[u.role] ?? u.role}</span>
              <button
                type="button"
                onClick={() => openEdit(u)}
