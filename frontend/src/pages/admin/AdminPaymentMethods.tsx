          ...
          {list.map((m) => (
            <li key={m.id} className="card p-4 flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button type="button" onClick={() => startEdit(m)} className="p-2 rounded-lg bg-surface-elevated text-gray-400 hover:text-white" title="Sửa">
                  <Pencil className="w-4 h-4" />
                </button>
                <button
                  type="button"
                  onClick={() => toggleActive(m.id, !m.active)}
                  className={`p-2 rounded-lg bg-surface-elevated ${m.active ? 'text-red-400 hover:bg-red-500/10' : 'text-green-400 hover:bg-green-500/10'}`}
                >
                  {m.active ? 'Tắt' : 'Bật'}
                </button>
              </div>
