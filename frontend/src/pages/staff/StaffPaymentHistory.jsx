...
export default function StaffPaymentHistory() {
  ...
  const openDetail = (order) => {
    ...
    if (method === 'momo' || method === 'bank_transfer' || method === 'cash') setConfirmMethod(method)
    else setConfirmMethod('cash')
  }
            ...
            {orders.map((o) => (
              <tr key={o._id} className="border-b border-gray-700/40">
                ...
                <td className="p-3 text-gray-300">
                  {o.paymentMethod === 'momo' ? 'Momo' : o.paymentMethod === 'bank_transfer' ? 'Chuyển khoản ngân hàng' : 'Tiền mặt'}
                </td>
                <td className="p-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-surface-card border border-gray-700 p-4 space-y-3">
            ...
            <p className="text-sm text-gray-400">
              Phương thức:{' '}
              {detail.paymentMethod === 'momo' ? 'Momo' : detail.paymentMethod === 'bank_transfer' ? 'Chuyển khoản ngân hàng' : 'Tiền mặt'}
            </p>
            <p className="text-sm text-gray-400">
            ...
            <div
              className={`rounded-lg p-3 space-y-2 ${
                detail.paymentStatus !== 'paid' ? 'border border-amber-500/30 bg-amber-500/10' : 'border border-gray-700 bg-surface-elevated/40'
              }`}
            >
              <p className={`text-sm ${detail.paymentStatus !== 'paid' ? 'text-amber-200' : 'text-gray-300'}`}>
                {detail.paymentStatus !== 'paid' ? 'Chọn phương thức trước khi xác nhận thanh toán' : 'Có thể chỉnh sửa phương thức thanh toán của đơn đã hoàn thành'}
              </p>
              <select
                value={confirmMethod}
            ...
            <div className="flex flex-wrap gap-2 justify-end">
              <button type="button" onClick={() => printInvoice(detail)} className="btn-secondary" disabled={saving}>
                Xuất hóa đơn
              </button>
            </div>
          </div>
