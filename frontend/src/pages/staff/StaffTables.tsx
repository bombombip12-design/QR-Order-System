...
export default function StaffTables() {
  ...
  const load = () => {
    tablesApi
      ...
      .then((list) => {
        setTables(list)
        return Promise.all(list.map((t) => ordersApi.byTable(t._id))).then(
          (orderLists) => {
  ...
  const statusInfo = (status?: Table['status']) => {
    if (status === 'occupied') return { label: 'Đang có khách', bg: 'bg-red-500/10', border: 'border-red-500/40', text: 'text-red-300' }
    if (status === 'reserved') return { label: 'Đã đặt trước', bg: 'bg-amber-500/10', border: 'border-amber-500/40', text: 'text-amber-300' }
    return { label: 'Trống', bg: 'bg-green-500/10', border: 'border-green-500/40', text: 'text-green-300' }
  }

  const openEdit = (t: Table) => {
    setEditing(t)
    setEditStatus((t.status || 'available') as Table['status'])
  }
  ...
  const saveEdit = async () => {
    ...
    setSaving(true)
    try {
      await tablesApi.update(editing._id, {
        status: editStatus,
      })
      setEditing(null)
      load()
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {tables.map((table) => {
        ...
        const effectiveStatus: Table['status'] = pendingCount > 0 ? 'occupied' : 'available'
        return (
          <div
            key={table._id}
            className={`card p-5 text-left transition-colors border ${statusInfo(effectiveStatus).bg} ${statusInfo(effectiveStatus).border}`}
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              {pendingCount > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-brand-500/20 text-brand-400 text-sm">
                  {pendingCount} đơn
                </span>
              )}
            </div>
            <p className={`text-sm mt-2 ${statusInfo(effectiveStatus).text}`}>{statusInfo(effectiveStatus).label}</p>
            <p className="text-sm text-gray-400 mt-1">{activeOrders.length} đơn hàng</p>
            <p className="text-sm text-gray-400">
              Sức chứa: {table.capacity ?? table.seats ?? 0} người
            </p>
          </div>
        )
      })}
