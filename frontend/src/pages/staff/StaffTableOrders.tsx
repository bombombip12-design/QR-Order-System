...
export default function StaffTableOrders() {
  ...
  const refreshOrders = () => {
    ...
    ordersApi
      .byTable(tableId)
      .then((ords) => setOrders((ords || []).filter((o) => o.status !== 'cancelled' && o.paymentStatus !== 'paid')))
  }
                ...
                {order.items.map((oi, idx) => {
                  ...
                  return (
                    <li key={idx} className="flex justify-between text-sm">
                      <span className="text-gray-300">{name} × {oi.quantity}</span>
                    </li>
                  )
                })}
              </ul>
              {order.totalAmount != null && (
                <p className="text-brand-400 font-medium mb-3">
